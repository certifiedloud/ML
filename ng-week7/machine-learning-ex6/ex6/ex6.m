...
% lowestError = 9999;
% pairUsedInLowestError = [];
%  for i=1:length(testVals)
%    for j=1:length(testVals)
%     % try the current combo of vals
%       C = testVals(i)
%       sigma = testVals(j)
%       model= svmTrain(Xval, yval, C, @(x1, x2) gaussianKernel(x1, x2, sigma));
%       predictions = svmPredict(model, Xval);
%       finalError = mean(double(predictions ~= yval))
%       if (finalError < lowestError)
%         lowestError = finalError
%         pairUsedInLowestError = [C, sigma];
%       endif
%    endfor
% endfor
% disp(finalError)
% disp(pairUsedInLowestError)
